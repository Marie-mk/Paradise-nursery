...
import { removeItem, increaseQuantity, decreaseQuantity, selectCartItems, selectCartTotal } from '../store/CartSlice';
          ...
          <>
            {cartItems.map((item) => (
              <div className="cart-item-card" key={item.id}>
                ...
                <button className="btn-delete" onClick={() => dispatch(removeItem(item.id))}>🗑️</button>
              </div>
            ))}
